# -*- coding: utf-8 -*-
"""
CKGGNN Model for Traffic Speed Prediction with Knowledge Graph Context
Adapted from: /home/wangwenrui/shk/AgentCity/repos/CKGGNN/libcity/model/traffic_speed_prediction/CKGGNN.py

This model integrates spatial and temporal Knowledge Graph embeddings with
Graph Neural Networks for traffic speed prediction. It uses DCRNN-based
encoder-decoder architecture combined with multi-head attention for
contextual feature fusion.

Key components:
- GCONV: Graph convolution with diffusion process
- DCGRUCell: Diffusion Convolutional GRU Cell
- EncoderModel/DecoderModel: Seq2Seq encoder-decoder
- Multi-head attention for spatial/temporal context fusion

Required config parameters:
- kg_embed_dim: Dimension of KG embeddings
- time_intervals: Time interval settings
- atten_type: Attention type ('head')
- head_type: Head type ('FeaSeqPlus')
...
- kg_context: KG context type ('spat', 'temp', or both)
- spat_model_used, spat_attr_used, spat_link_attr, spat_buffer_attr
- temp_model_used, temp_attr_used, temp_time_attr, temp_link_attr
- kg_weight: Weight method ('times', 'add', 'concat')
"""
...
class CKGGNN(AbstractTrafficStateModel, Seq2SeqAttrs):
    ...
    def encoder(self, inputs):
        ...
        for t in range(self.input_window):
            ...
            # encoder_hidden_state: encoder's multi-layer GRU hidden states (num_layers, batch_size, self.hidden_state_size)
