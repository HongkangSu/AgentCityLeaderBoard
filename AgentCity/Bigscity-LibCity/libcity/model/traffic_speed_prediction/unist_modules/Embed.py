...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, dropout=0.1, t_patch_size=1, patch_size=1, size1=48, size2=7):
        """
        Data Embedding module for UniST.
        ...
        Args:
            ...
            size1: Hour embedding size (48 for 30-min intervals, 24 for 1-hour intervals)
            size2: Weekday embedding size (7)
        """
        super(DataEmbedding, self).__init__()
        self.t_patch_size = t_patch_size
        self.patch_size = patch_size
        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model,
                                               t_patch_size=t_patch_size, patch_size=patch_size)
        self.temporal_embedding = TemporalEmbedding(t_patch_size=t_patch_size, d_model=d_model,
                                                     hour_size=size1, weekday_size=size2)
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, x, x_mark, is_time=1):
        '''
        x: N, T, C, H, W  or  N, C, T, H, W (depending on format)
        x_mark: N, T, D
        '''
        # Handle both input formats
        if x.dim() == 5:
            if x.shape[1] == 1:  # N, C, T, H, W format
                x = x.permute(0, 2, 1, 3, 4)  # -> N, T, C, H, W

        N, T, C, H, W = x.shape
        # Permute to N, C, T, H, W for Conv3d
        x_conv = x.permute(0, 2, 1, 3, 4)
        TokenEmb = self.value_embedding(x_conv)
        TimeEmb = self.temporal_embedding(x_mark)

        spatial_factor = (H // self.patch_size) * (W // self.patch_size)
        assert TokenEmb.shape[1] == TimeEmb.shape[1] * spatial_factor
