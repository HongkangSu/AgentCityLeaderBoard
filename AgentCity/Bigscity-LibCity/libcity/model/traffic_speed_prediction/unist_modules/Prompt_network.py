...
class Sptial_prompt(nn.Module):
    ...
    def __init__(self, num_memory, memory_dim, in_channels, conv_num):
        super().__init__()

        self.conv = nn.ModuleList()
        self.conv.append(nn.Conv2d(in_channels=in_channels, out_channels=memory_dim, kernel_size=1, padding='same'))
        for i in range(conv_num - 1):
            self.conv.append(nn.Conv2d(in_channels=in_channels, out_channels=memory_dim, kernel_size=2**(i+1)+1, padding='same'))

        self.spatial_memory = Memory(num_memory, memory_dim)
...
class Temporal_prompt(nn.Module):
    ...
    def __init__(self, num_memory, memory_dim):
        ...
        self.temporal_memory = Memory(num_memory, memory_dim)

        encoder_layer = nn.TransformerEncoderLayer(d_model=memory_dim, nhead=4, dim_feedforward=memory_dim, batch_first=True)

        self.c_encoder = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=1)

        self.p_encoder = nn.TransformerEncoder(encoder_layer=copy.deepcopy(encoder_layer), num_layers=1)
...
class Prompt_ST(nn.Module):
    ...
    def __init__(self, num_memory_spatial, num_memory_temporal, memory_dim, in_channels, conv_num):
        super().__init__()

        self.spatial_prompt = Sptial_prompt(num_memory_spatial, memory_dim, in_channels, conv_num)
        self.temporal_prompt = Temporal_prompt(num_memory_temporal, memory_dim)
