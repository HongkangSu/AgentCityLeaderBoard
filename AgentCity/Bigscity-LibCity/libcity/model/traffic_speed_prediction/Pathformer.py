...
try:
    ...
    # Some parts of Pathformer require einops
    rearrange = reduce = repeat = None
...
class series_decomp_multi(nn.Module):
    ...
    def forward(self, x):
        ...
        softmax_weight = nn.Softmax(-1)(self.layer(x.unsqueeze(-1)))
        moving_mean = torch.sum(moving_mean * softmax_weight, dim=-1)
        res = x - moving_mean
...
class FourierLayer(nn.Module):
    ...
    def dft_forward(self, x):
        ...
        idft_mat = torch.tensor(np.power(omega, i * j) / T, dtype=torch.complex64)
        x_freq = torch.einsum('ft,btd->bfd', [dft_mat.to(x.device), x.to(torch.complex64)])
        if T % 2 == 0:
        ...
        attn = torch.einsum('bofd,bftd->botd', [idft_mat.to(x.device), dft_mat.to(x.device)]).real
...
def positional_encoding(pe, learn_pe, q_len, d_model):
    if pe == 'zeros':
        ...
        # Simplified for LibCity adaptation
...
class WeightGenerator(nn.Module):
    def __init__(self, in_dim, out_dim, mem_dim, num_nodes, factorized, number_of_weights=4):
        ...
        if self.factorized:
            ...
            self.mem_dim_actual = 10
            self.P = nn.ParameterList(
                [nn.Parameter(torch.Tensor(in_dim, self.mem_dim_actual), requires_grad=True) for _ in range(number_of_weights)])
            self.Q = nn.ParameterList(
                [nn.Parameter(torch.Tensor(self.mem_dim_actual, out_dim), requires_grad=True) for _ in range(number_of_weights)])
            self.B = nn.ParameterList(
                [nn.Parameter(torch.Tensor(self.mem_dim_actual ** 2, out_dim), requires_grad=True) for _ in range(number_of_weights)])
        else:
            self.P = nn.ParameterList(
    ...
    def forward(self):
        if self.factorized:
            ...
            memory = memory.view(self.num_nodes, self.mem_dim_actual, self.mem_dim_actual)
...
# --- Main Model ---
class Pathformer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        super().__init__(config, data_feature)
        self.device = config.get('device', torch.device('cpu'))
        self.num_nodes = data_feature.get('num_nodes', 1)
        self.seq_len = config.get('input_window', 12)
        self.pre_len = config.get('output_window', 12)
        self.layer_nums = config.get('layer_nums', 3)
        self.k = config.get('k', 2)
        self.num_experts_list = config.get('num_experts_list', [4, 4, 4])
        self.patch_size_list = config.get('patch_size_list', [16, 12, 8])
        ...
        for num in range(self.layer_nums):
            self.AMS_lists.append(
                AMS(self.seq_len, self.seq_len, self.num_experts_list[num], self.device,
    ...
    def forward(self, batch):
        ...
        return out.unsqueeze(-1), balance_loss
    ...
    def calculate_loss(self, batch):
        y_true = batch['y']
        y_predicted, balance_loss = self.forward(batch)
        y_true = y_true[..., :1]
        loss = F.mse_loss(y_predicted, y_true)
