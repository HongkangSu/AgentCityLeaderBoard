# -*- coding: utf-8 -*-
"""
Knowledge Graph Embedding Template Functions for CKGGNN Model

This module provides utility functions for generating and processing
Knowledge Graph embeddings for spatial and temporal contexts.

Adapted from: /home/wangwenrui/shk/AgentCity/repos/CKGGNN/libcity/pipeline/embedkg_template.py

Key functions:
- generate_spatial_kg: Generate spatial KG triples factory
- generate_temporal_kg: Generate temporal KG triples factory
- generate_kgsub_spat: Generate spatial sub-KG with embeddings
- generate_kgsub_temp: Generate temporal sub-KG with embeddings
- kg_embedding: Train KG embedding model using pykeen

Dependencies:
- pykeen: For KG embedding training
"""
...
def obatin_spatial_pickle(config, logger):
    """Load spatial KG data from pickle file."""
...
def obatin_temporal_pickle(config, logger):
    """Load temporal KG data from pickle file."""
...
def add_factdict_list(dict_fact):
    """Add all facts from dictionary to a list."""
    list_fact = []
...
def add_factdict_list_bufferfilter(dict_fact, buffer_start, buffer_end):
    """Add facts from dictionary filtered by buffer range."""
    list_fact = []
    for _key in dict_fact:
...
def generate_spatial_kg(config, logger):
    """
    Generate spatial Knowledge Graph triples factory.
    ...
    Returns:
        TriplesFactory: PyKEEN triples factory for spatial KG
    """
    spat_pickle_file = config.get('spat_pickle_file')
...
def add_factdict_list_sub(dict_kgsub, dictKG_spatial, str_flag, logger):
    """Add facts from spatial KG to sub-KG dictionary with attribute values."""
    # basic pars
...
def add_factdict_list_bufferfilter_sub(dict_kgsub, dictKG_spatial, str_flag, buffer_str, logger):
    """Add facts from spatial KG filtered by buffer range to sub-KG dictionary."""
    # basic pars
    dict_fact = dictKG_spatial[str_flag + '_spat']
    dict_attr = dictKG_spatial[str_flag + '_attr']
    road_list = list(dict_fact.keys())
...
def generate_kgsub_spat(config, logger, dictKG_spatial):
    """
    Generate spatial sub-KG with embeddings for each road.

    Args:
        config: Configuration dictionary
        logger: Logger instance
        dictKG_spatial: Spatial KG dictionary

    Returns:
        dict: Sub-KG dictionary {road_id: {road/poi/land/link: [fact]}}
    """
...
def add_tempfact_time(dictKG_temporal, datetime_list):
    """Add time-related facts (hour, day, weekday) from temporal KG."""
    list_fact = []
...
def add_tempdict_fact(dictKG_temporal, datetime_list, time_used, link_used, name_sur, name_key):
    """Add temporal facts with time averaging and linking."""
    list_fact = []
...
def generate_temporal_kg(config, logger, kg_logger=True):
    """
    Generate temporal Knowledge Graph triples factory.

    Args:
        config: Configuration dictionary
        logger: Logger instance
        kg_logger: Whether to log KG generation details

    Returns:
        TriplesFactory: PyKEEN triples factory for temporal KG
    """
    temp_pickle_file = config.get('temp_pickle_file')
...
def add_tempfact_time_sub(dict_kgsub, dictKG_temporal, temp_datetime, str_flag):
    """Add time-related facts to temporal sub-KG with normalized values."""
...
def add_tempdict_fact_sub(logger, dict_kgsub, dictKG_temporal, temp_datetime, time_used, link_used, name_sur, name_key):
    """Add temporal facts with time averaging to sub-KG dictionary."""
...
def add_tempdict_fact_sub_notcover(logger, dict_kgsub, dictKG_temporal, temp_datetime, time_used, link_used, name_sur, name_key, temp_datetime_list, kg_weight_temp):
    """Add temporal facts with time averaging, handling missing data."""
    max_value, min_value = 0, 0
    if name_sur == 'jam':
        max_value, min_value = 10.0, 0.0
    if name_sur == 'tprt':
...
def generate_kgsub_temp(config, logger, dictKG_temporal, temp_datetime, kg_logger=False):
    """
    Generate temporal sub-KG with embeddings for each road at a specific datetime.

    Args:
        config: Configuration dictionary
        logger: Logger instance
        dictKG_temporal: Temporal KG dictionary {time: {id: {keys: value}}}
        temp_datetime: Target datetime
        kg_logger: Whether to log details

    Returns:
        dict: Sub-KG dictionary {road_id: {time/jam/tprt/rain/wind: [fact]}}
    """
...
def generate_kgsub_temp_notcover(config, logger, dictKG_temporal, temp_datetime, temp_datetime_list, kg_weight_temp, kg_logger=False):
    """
    Generate temporal sub-KG handling missing data points.

    Args:
        config: Configuration dictionary
        logger: Logger instance
        dictKG_temporal: Temporal KG dictionary
        temp_datetime: Target datetime
        temp_datetime_list: List of available datetimes
        kg_weight_temp: Weight method for handling missing data
        kg_logger: Whether to log details

    Returns:
        dict: Sub-KG dictionary
    """
    # dict_kgsub; dictKG_temporal: {time: {id: keys: {value}}}
    dict_kgsub = {}
...
def kg_entity_id_from_name(tf_facts, ctx_name):
    """Get entity IDs from triples factory matching a name pattern."""
    entity_list = []
    entityid_list = []
...
def kg_entity_id_from_list(tf_facts, lab_list):
    """Get entity IDs from triples factory for a list of labels."""
    entityid_list = []
    entity_label = tf_facts.entity_labeling.label_to_id
...
def kg_embedding(tf_facts, config, logger, model_used, kg_logger=False):
    """
    Train KG embedding model using PyKEEN pipeline.

    Args:
        tf_facts: TriplesFactory for training
        config: Configuration dictionary
        logger: Logger instance
        model_used: KG embedding model name (e.g., 'ComplEx', 'KG2E', 'AutoSF')
        kg_logger: Whether to log training details

    Returns:
        tuple: (entity_embedding_numpy, relation_embedding_numpy)
    """
