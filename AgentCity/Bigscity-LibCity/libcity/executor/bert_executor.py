"""
BERT Base Executor for START model.
Migrated from: /repos/START/libcity/executor/bert_executor.py

This executor provides BERT-style masked language model training with:
- Token-level prediction and accuracy tracking
...
class BertBaseExecutor(STARTAbstractExecutor):
    """
    BERT-style executor for masked language model training.

    Features:
    - NLLLoss for masked token prediction
    - Checkpoint loading from pre-trained models
    - Parameter unloading for fine-tuning
    - Accuracy tracking for masked positions
    """
    ...
    def _valid_parameter(self, k):
        """Check if parameter should be excluded from loading"""
    ...
    def load_model_with_initial_ckpt(self, initial_ckpt):
        """
        Load model weights from initial checkpoint with parameter filtering

        Args:
            initial_ckpt(str): Path to initial checkpoint file
        """
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        Args:
            eval_dataloader: Validation/test data loader
            epoch_idx: Current epoch index
            mode: 'Eval' or 'Test'
