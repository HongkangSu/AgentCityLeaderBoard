"""
Contrastive Split MLM Executor for START model.
Migrated from: /repos/START/libcity/executor/contrastive_split_mlm_executor.py

This executor extends ContrastiveMLMExecutor to handle split data format where
contrastive views and MLM data have separate padding masks and temporal matrices.
"""
...
class ContrastiveSplitMLMExecutor(ContrastiveMLMExecutor):
    """
    Extended Contrastive MLM Executor for split data format.

    This executor handles a more complex batch format where:
    - Contrastive views have their own padding masks and temporal matrices
    - MLM data has separate padding masks and temporal matrices
    - All tensors are passed explicitly to the model
    """
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        """
        Train one epoch with split data format.

        The batch contains:
        - contra_view1, contra_view2: Contrastive learning views
        - padding_masks1, padding_masks2: Padding masks for contrastive views
        - batch_temporal_mat1, batch_temporal_mat2: Temporal matrices for contrastive views
        - X: Masked input for MLM
        - targets, target_masks: MLM targets and masks
        - padding_masks: Padding mask for MLM
        - batch_temporal_mat: Temporal matrix for MLM

        Returns:
            tuple: (average loss, average location accuracy)
        """
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        """
        Validate one epoch with split data format.
