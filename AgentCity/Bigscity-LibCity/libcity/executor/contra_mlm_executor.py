"""
Contrastive MLM Executor for START model.
Migrated from: /repos/START/libcity/executor/contra_mlm_executor.py

This executor combines contrastive learning with masked language modeling (MLM).
It supports:
- Joint training with contrastive and MLM objectives
- Separate loss tracking for each objective
- Accuracy tracking for masked token prediction
"""
...
class ContrastiveMLMExecutor(ContrastiveExecutor):
    """
    Combined Contrastive Learning and Masked Language Model Executor.

    Features:
    - NLLLoss for masked token prediction
    - Weighted combination of MLM and contrastive losses
    - Separate tracking of MLM accuracy and contrastive loss
    - Support for test-time evaluation with evaluator
    """
    ...
    def _cal_loss(self, pred, targets, targets_mask):
        ...
        Args:
            ...
            targets_mask: Mask for active positions (batch_size, seq_len)

        Returns:
            tuple: (mean_loss, batch_loss, num_active)
        """
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        """
        Train model with combined contrastive and MLM objectives.
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        """
        Train one epoch with combined losses.
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        """
        Validate one epoch with combined losses.
