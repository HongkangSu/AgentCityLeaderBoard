"""
Contrastive Executor for START model.
Migrated from: /repos/START/libcity/executor/contrastive_executor.py

This executor provides contrastive learning training with:
...
class ContrastiveExecutor(BertBaseExecutor):
    """
    Contrastive learning executor supporting multiple loss types.

    Features:
    - SimCLR, SimSCE, ConSERT contrastive loss functions
    - Alignment and uniformity loss for representation quality
    ...
    def align_loss(self, x, y, alpha=2):
        """
        Compute alignment loss between two representations.

        Args:
            x: First representation tensor (batch_size, d_model)
            y: Second representation tensor (batch_size, d_model)
            alpha: Power for distance computation
    ...
    def uniform_loss(self, x, t=2):
        ...
        Args:
            x: Representation tensor (batch_size, d_model)
            t: Temperature for uniformity computation
    ...
    def align_uniform(self, x, y):
        """
        Compute combined alignment and uniformity loss.
        ...
        Returns:
            tuple: (combined loss, alignment loss value, uniformity loss value)
        """
    ...
    def _contrastive_loss(self, z1, z2, loss_type):
        ...
        Args:
            z1: First view representations (batch_size, d_model)
            z2: Second view representations (batch_size, d_model)
            loss_type: Type of contrastive loss ('simsce', 'simclr', 'consert')
    ...
    def _contrastive_loss_consert(self, z1, z2):
        ...
        if self.similarity == 'inner':
            ...
            logits_aa = F.cosine_similarity(z1.unsqueeze(1), hidden1_large.unsqueeze(0), dim=-1) / self.temperature
            logits_bb = F.cosine_similarity(z2.unsqueeze(1), hidden2_large.unsqueeze(0), dim=-1) / self.temperature
            logits_ab = F.cosine_similarity(z1.unsqueeze(1), hidden2_large.unsqueeze(0), dim=-1) / self.temperature
            logits_ba = F.cosine_similarity(z2.unsqueeze(1), hidden1_large.unsqueeze(0), dim=-1) / self.temperature
        else:
            logits_aa = torch.matmul(z1, hidden1_large.transpose(0, 1)) / self.temperature
            logits_bb = torch.matmul(z2, hidden2_large.transpose(0, 1)) / self.temperature
            logits_ab = torch.matmul(z1, hidden2_large.transpose(0, 1)) / self.temperature
            logits_ba = torch.matmul(z2, hidden1_large.transpose(0, 1)) / self.temperature
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        Args:
            ...
            test_dataloader(torch.Dataloader): Optional test dataloader
        """
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            if self.test_align_uniform or self.train_align_uniform:
                align_uniform_loss, align_loss_val, uniform_loss_val = self.align_uniform(z1, z2)
            ...
            if self.test_align_uniform or self.train_align_uniform:
                post_fix['align_loss'] = align_loss_val
                post_fix['uniform_loss'] = uniform_loss_val
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        """Validate one epoch for contrastive learning"""
